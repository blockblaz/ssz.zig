...
test "Validator struct hash tree root" {
    ...
    // Validate against expected hash from fastssz
...
test "Individual Validator serialization and hash - fastssz compatibility" {
    // Test individual validator matching fastssz test vectors
    const validator = Validator{
        .pubkey = [_]u8{0x01} ** 48,
        .withdrawal_credentials = [_]u8{0x02} ** 32,
        .effective_balance = 32000000000,
    ...
    // Validate against expected bytes from fastssz
...
test "Zeam-style List/Bitlist usage with tree root stability" {
    ...
    // Test hash tree root determinism and validate against fastssz
    ...
    // Validate against expected hashes from fastssz
    ...
    // NOTE: State hash temporarily disabled - known difference in implementation for bitlists
    ...
    // NOTE: State hash validation temporarily disabled - requires correction of bitlist hash implementation
